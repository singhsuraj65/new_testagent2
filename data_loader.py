...
- Unit price calculation for inventory cost impact
"""
...
def build_material_summary(data: dict) -> pd.DataFrame:
    ...
    for mat in df_lt["Material"].unique():
        ...
        # ----- NEW: Calculate unit price from latest period -----
        latest_row = lt_sub.iloc[-1] if len(lt_sub) > 0 else None
        if latest_row is not None and latest_row["Gross Stock"] > 0:
            unit_price = latest_row["Gross Total Val (USD)"] / latest_row["Gross Stock"]
        else:
            # fallback to Std Cost from current inventory
            unit_price = ci_row["Std Cost (USD)"].values[0] if len(ci_row) > 0 and pd.notna(ci_row["Std Cost (USD)"].values[0]) else 0
        # ---------------------------------------------------------
        ...
        # Lead time logic (prioritise inhouse_time as per stakeholder)
        if inhouse_time > 0:
        ...
        rows.append({
            ...
            "unit_price": round(unit_price, 2),   # <-- new field for inventory cost impact
        })
