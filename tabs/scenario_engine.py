        ...
        with dr:
            ...
            if run_dis or st.session_state.get("dis_ran"):
                ...
                if breached_materials:
                    ...
                    <p>⚠️ <strong>Supply disruption of {disruption_days} days would cause stockouts for {len(breached_materials)} material(s).</strong></p>
                    ...
                    manual_disruption_html = f"<p>✅ <strong>Supply disruption of {disruption_days} days would NOT cause any stockout.</strong> Current stock levels are sufficient to cover the freeze period.</p>"
    ...
    with rep_tab:
        ...
        if len(pds_lbl) > 4:
            ...
            if st.button("↺  Replay this period", key="rpb"):
                ...
                # Copy and reset index to avoid integer/string mix‑up
                rd = shrp.iloc[idx:idx + 6].copy()
                ...
                # Use numeric positions for vlines – no string arithmetic
                br2 = rd[rd["Gross Stock"] < max(ssr, 1)]
                if len(br2) > 0:
                    try:
                        breach_pos = br2.index[0]          # integer (0‑5)
                        prev_pos = max(0, breach_pos - 1)  # integer
                        fr.add_vline(x=breach_pos, line_color="#EF4444", line_dash="dash",
                                     annotation_text="⛔ Breach", annotation_font_color="#EF4444")
                        fr.add_vline(x=prev_pos, line_color="#22C55E", line_dash="dash",
...
# # # #                 # ── Enhanced manual explanation (used when LLM fails) ──────────
# # # #                 expected_breach = bi.get("Expected")
# # # #                 low_breach = bi.get("Low (−40%)")
# # # #                 high_breach = bi.get("High (+60%)")
                
# # # #                 # Build detailed manual verdict
# # # #                 manual_lines = []
# # # #                 # Expected scenario
# # # #                 if expected_breach is not None:
# # # #                     manual_lines.append(f"⚠️ **Expected demand scenario:** Stockout would occur in **month {expected_breach}**.")
# # # #                     manual_lines.append(f"   - Current stock: {stk} units | Safety stock: {ss} units.")
# # # #                     manual_lines.append(f"   - **Action:** Place an order immediately or before month {expected_breach} to avoid disruption.")
# # # #                 else:
# # # #                     manual_lines.append(f"✅ **Expected demand scenario:** Stock remains **above safety stock** for all 6 months.")
# # # #                     manual_lines.append(f"   - Current stock: {stk} units | Safety stock: {ss} units.")
# # # #                     manual_lines.append(f"   - **Action:** Continue monitoring; no urgent order needed.")
                
# # # #                 # Low demand scenario
# # # #                 if low_breach is not None:
# # # #                     manual_lines.append(f"📉 **Low demand (−40%):** Stockout in month {low_breach}.")
# # # #                 else:
# # # #                     manual_lines.append(f"📉 **Low demand (−40%):** No stockout.")
                
# # # #                 # High demand scenario
# # # #                 if high_breach is not None:
# # # #                     manual_lines.append(f"📈 **High demand (+60%):** Stockout in month {high_breach}.")
# # # #                 else:
# # # #                     manual_lines.append(f"📈 **High demand (+60%):** No stockout.")
                
# # # #                 # Summary recommendation
# # # #                 if expected_breach is not None:
# # # #                     manual_lines.append(f"\n**Summary:** Under expected demand, you have only {expected_breach} month(s) of cover. The high-demand scenario shows risk as early as month {high_breach if high_breach else 'N/A'}. **Consider increasing safety stock or placing a buffer order immediately.**")
# # # #                 else:
# # # #                     manual_lines.append(f"\n**Summary:** Under expected demand, you have more than 6 months of cover. The high-demand scenario shows risk in month {high_breach if high_breach else 'N/A'}. **No immediate action required, but monitor demand trends.**")
...
# # # #                             {"quantity": oq, "timing_days": ot} if oon else None,
# # # #                         )
# # # #                     # Replace the generic LLM failure message with the enhanced manual explanation
# # # #                     if sv.get("simulation_verdict") == "Simulation completed, but ARIA could not generate a verdict. Please review the graph manually.":
# # # #                         sv["simulation_verdict"] = manual_explanation
# # # #                     urg  = sv.get("urgency", "MONITOR")
# # # #                     uc   = {"ACT TODAY": "#EF4444", "ACT THIS WEEK": "#F59E0B", "MONITOR": ORANGE, "SAFE": "#22C55E"}.get(urg, ORANGE)
...
# # # #                         f"<span style='font-size:12px;font-weight:800;color:{uc};'>{urg}</span>"
# # # #                         f"<span class='chip'>Min order: {sv.get('min_order_recommended', '—')} units</span></div>"
# # # #                         f"<div class='ib' style='white-space:pre-line;'>{sv.get('simulation_verdict', '')}</div></div>",
# # # #                         unsafe_allow_html=True,
# # # #                     )
# # # #                 elif rsim and not st.session_state.azure_client:
# # # #                     # No Azure client – show enhanced manual explanation directly
# # # #                     st.markdown(
# # # #                         f"<div class='ic' style='margin-top:10px;'><div class='il'>◈ ARIA Verdict (Manual)</div>"
# # # #                         f"<div class='ib' style='white-space:pre-line;'>{manual_explanation}</div></div>",
# # # #                         unsafe_allow_html=True,
# # # #                     )
...
# # #                 # Build detailed HTML manual verdict for Demand Shock
# # #                 expected_breach = bi.get("Expected")
# # #                 low_breach = bi.get("Low (−40%)")
# # #                 high_breach = bi.get("High (+60%)")
...
# # #                 html_lines = []
# # #                 if expected_breach is not None:
# # #                     html_lines.append(f"<p>⚠️ <strong>Expected demand scenario:</strong> Stockout would occur in <strong>month {expected_breach}</strong>.</p>")
# # #                     html_lines.append(f"<p style='margin-left:20px;'>Current stock: {stk} units | Safety stock: {ss} units.<br>")
# # #                     html_lines.append(f"<strong>Action:</strong> Place an order immediately or before month {expected_breach} to avoid disruption.</p>")
# # #                 else:
# # #                     html_lines.append(f"<p>✅ <strong>Expected demand scenario:</strong> Stock remains above safety stock for all 6 months.</p>")
...
# # #                 html_lines.append("<hr style='margin:8px 0;'>")
# # #                 if low_breach is not None:
# # #                     html_lines.append(f"<p>📉 <strong>Low demand (−40%):</strong> Stockout in month {low_breach}.</p>")
# # #                 else:
# # #                     html_lines.append(f"<p>📉 <strong>Low demand (−40%):</strong> No stockout.</p>")
                
# # #                 if high_breach is not None:
# # #                     html_lines.append(f"<p>📈 <strong>High demand (+60%):</strong> Stockout in month {high_breach}.</p>")
# # #                 else:
# # #                     html_lines.append(f"<p>📈 <strong>High demand (+60%):</strong> No stockout.</p>")
